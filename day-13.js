...
function foldOverX655() {
  let newDots = [];
  let dotsObj = getDotsObj(DOTS);
  for (let i = 0, x, y, newX; i < DOTS.length; i++) {
    [x,y] = DOTS[i];
    if (x < 655) {
      newDots.push([x,y]);
    } else {
      newX = 655 - (x - 655);
      if (!dotsObj[newX] || !dotsObj[newX][y]) newDots.push([newX,y]);
    }
  }
  return newDots;
}
...
function fold(dotsArr, foldAxis, valueToFoldOn) {
  let newDots = [];
  let dotsObj = getDotsObj(dotsArr);
  for (let i = 0, x, y, newValue; i < dotsArr.length; i++) {
    ...
    if (foldAxis === "x") {
      if (x < valueToFoldOn) {
        ...
        if (!dotsObj[newValue] || !dotsObj[newValue][y]) newDots.push([newValue,y]);
      }
    } else if (foldAxis === "y") {
      if (y < valueToFoldOn) {
        newDots.push([x,y]);
        ...
        if (!dotsObj[x][newValue]) newDots.push([x,newValue]);
      }
    }
  }
  return newDots;
}
console.assert(fold(DOTS, "x", 655).length === 666);
...
function foldAll() {
  ...
  for (let i = 0, foldAxis, valueToFoldOn; i < FOLDS.length; i++) {
    ...
    dots = fold(dots, foldAxis, valueToFoldOn);
  }
  return dots;
}
let foldedDots = foldAll();

function visualizeDots(dotsArr) {
  let maxX = 0, maxY = 0;
  for (let i = 0, x, y; i < dotsArr.length; i++) {
    [x,y] = dotsArr[i];
    if (x > maxX) maxX = x;
